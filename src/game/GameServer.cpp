...
void GameServer::applyDamage(Player& p, float amount, const char* cause) {
    ...
    // armor reduction (plan5 79)
    int armor = totalArmorPoints(p.inv);
    float reduced = amount * (1.0f - std::min(20, armor) * 0.04f);
    // enchant protection placeholder: no enchant data yet, EPF=0
    // resistance effect
