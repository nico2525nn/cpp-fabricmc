...
const KeySpec* findKey(std::string_view name, bool propertySource) {
    const std::string lowered = asciiLower(std::string(name));
    for (const auto& spec : kKeySpecs) {
        if ((propertySource ? spec.property : spec.commandLine) &&
            asciiLower(std::string(spec.name)) == lowered) {
            return &spec;
        }
    }
    return nullptr;
...
void applyKnownValue(ServerConfig& config, std::string_view key, std::string_view value,
    ...
    applyValue(config, spec->key, value, diagnostics, key);
}
