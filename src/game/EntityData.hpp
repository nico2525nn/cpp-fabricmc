...
// Minimal per plan5: {type, attributes:{max_health,movement_speed}, spawning:{biomes,light_level,structures}, brain:{behaviors:[{type,priority}]}, equipment, loot}
#pragma once
#include <string>
#include <vector>
    ...
    void loadDirectory(const std::string& dir){
        ...
        for(auto &e: fs::directory_iterator(dir,ec)){
            ...
            try{
                ...
                if(brain.type==json::Value::Type::Obj){ const auto &bh=brain.at("behaviors"); if(bh.type==json::Value::Type::Arr) for(auto &b: bh.arr){ EntityDataDef::Behavior beh; beh.type=b.at("type").asStr(); beh.priority=b.at("priority").asInt(0); if(!beh.type.empty()) d.behaviors.push_back(std::move(beh)); } }
