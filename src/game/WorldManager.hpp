...
public:
    ...
    // forwarding hooks (implemented in GameServer via delegation)
    ...
    // chunk unload decision extracted from GameServer::chunksUnloadTick
    static bool shouldUnload(int32_t cx, int32_t cz, double px, double pz, double unloadDist2) {
        ...
        return dx*dx+dz*dz >= unloadDist2;
    }
