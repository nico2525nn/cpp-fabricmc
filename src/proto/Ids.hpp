...
constexpr std::uint8_t QueryBlockEntityTag = 0x01; // query_block_nbt {transactionId, position}
constexpr std::uint8_t SelectBundleItem = 0x02;    // plan45 B6 W-10(d): {slotId varint, selectedItemIndex varint}
...
constexpr std::uint8_t Settings = 0x0C;          // plan45 B6 W-05: packet_common_settings (config 0x00 same shape)
constexpr std::uint8_t TabComplete = 0x0D;        // command suggestion request
constexpr std::uint8_t ConfigurationAcknowledged = 0x0E; // plan45 B6 W-10(d): empty (transfer hook)
...
constexpr std::uint8_t SetSlotState = 0x12;      // plan45 B6 W-10(d): {slot_id, window_id:ContainerID, state}
constexpr std::uint8_t CookieResponse = 0x13;
constexpr std::uint8_t CustomPayload = 0x14;
constexpr std::uint8_t DebugSampleSubscription = 0x15; // plan45 B6 W-10(d): {type varint}
constexpr std::uint8_t EditBook = 0x16;          // plan45 B6 W-09: {hand, pages[], title?}
constexpr std::uint8_t QueryEntityNbt = 0x17;    // plan45 B6 W-10(d): {transactionId, entityId}
constexpr std::uint8_t UseEntity = 0x18;
constexpr std::uint8_t GenerateStructure = 0x19; // plan45 B6 W-09: {location, levels, keepJigsaws}
constexpr std::uint8_t KeepAlive = 0x1A;
constexpr std::uint8_t LockDifficulty = 0x1B;    // plan45 B6 W-10(d): {locked bool}
...
constexpr std::uint8_t SteerBoat = 0x21;         // plan45 B6 W-10(a): {leftPaddle bool, rightPaddle bool}
constexpr std::uint8_t PickItemFromBlock = 0x22; // plan45 B6 W-08: {position, includeData}
constexpr std::uint8_t PickItemFromEntity = 0x23;// plan45 B6 W-08: {entityId varint, includeData}
...
constexpr std::uint8_t Pong = 0x2B;              // plan45 B6 W-10(c): {id i32}
constexpr std::uint8_t RecipeBook = 0x2C;        // plan45 B6 W-08: {bookId, bookOpen, filterActive}
constexpr std::uint8_t DisplayedRecipe = 0x2D;   // plan45 B6 W-08: {recipeId varint}
constexpr std::uint8_t NameItem = 0x2E;          // plan45 B6 W-08: {name string} (anvil rename)
constexpr std::uint8_t ResourcePackReceive = 0x2F;// plan45 B6 W-10(b): {uuid, result varint}
constexpr std::uint8_t AdvancementTab = 0x30;    // plan45 B6 W-10(d): {action, tabId?}
constexpr std::uint8_t SetBeaconEffect = 0x32;   // plan45 B6 W-08: {primary? option, secondary? option}
constexpr std::uint8_t HeldItemSlot = 0x33;
constexpr std::uint8_t UpdateCommandBlock = 0x34;        // plan45 B6 W-09: {location,command,mode,flags}
constexpr std::uint8_t UpdateCommandBlockMinecart = 0x35;// plan45 B6 W-09: {entityId,command,track_output}
constexpr std::uint8_t SetCreativeModeSlot = 0x36;
constexpr std::uint8_t UpdateJigsaw = 0x37;      // plan45 B6 W-09: 8 fields
constexpr std::uint8_t UpdateStructureBlock = 0x38; // plan45 B6 W-09: 16 fields
...
constexpr std::uint8_t PlayerListHeader = 0x74;  // (plan45 B6: header/footer, currently unsent)
constexpr std::uint8_t TagQueryResponse = 0x75;  // plan45 B6 W-10(d): {transactionId varint, nbt} (F3+I answer)
...
constexpr std::uint8_t MapData = 0x2D;                // (a) sent (plan42 network: map_data 0x2D itemDamage+scale+locked+icons+columns)
constexpr std::uint8_t MoveMinecart = 0x31;           // (a) sent (plan42 network: move_minecart 0x31 eid+steps vec3f)
...
constexpr std::uint8_t SelectAdvancementTab = 0x4F;   // (a) sent (plan42 network: select_advancement_tab 0x4F id option<string>)
...
constexpr std::uint8_t UpdateViewDistance = 0x59;     // sent (plan46 O-03: login + Settings 0x0C confirm, server-clamped vd)
