...
| 8 | Structures | DONE | `StructureManager.cpp:32` + `StructurePlacer.cpp:18` | **plan12 §3 DONE:** `StructureSet {spacing,separation,salt,Linear/Triangular}` — `stronghold 32/5`, `mineshaft 10/5`, `monument 32/5 salt 10387313`, `mansion 80/20 salt 10387319`, `end_city 20/11`; template pieces `portal_room/corridor/prismarine 58×58×23/mansion 40×40`; jigsaw fallback `Trial Chambers/Pale Garden` still polish. |
...
| 19 | Pistons | DONE | `Redstone.cpp:597` | `MovingPiston` 2-tick + `isStickyBlock` + `sticksTogether slime≠honey` + 12-block BFS + `isUnpushable` + `PistonMove` sound; polish: honey/slime multi-block retract chain beyond 1 block still simplified. |
| 20 | Fluid solidify | DONE | `Fluids.cpp:13` + `BlockTickScheduler.cpp:453` | **plan12 §8 DONE:** `water+lava→cobble/obsidian/stone` + `WaterloggableHelper` `stairs/slab/fence/wall/trapdoor` (double slab `!waterloggable`) + `kindAt` waterlogged→`Water 0` + `KelpBehavior age25 10%` + `seagrass` fallback; polish: `kelp` only, no `tall_seagrass`. |
...
| 90 | Effects `EntityEffect 0x5E` | DONE | `MobEffects.hpp:14` + `GameServer.cpp:2265` | `Speed/Slowness→MOVEMENT_SPEED` + `HealthBoost→MAX_HEALTH` + `Invisibility/Glowing/Levitation` via `SetEntityMetadata 0x5D` flags + `UpdateAttributes 0x7C` 20t `applyEffectModifiers`; polish: `Levitation` gravity off still simplified. |
