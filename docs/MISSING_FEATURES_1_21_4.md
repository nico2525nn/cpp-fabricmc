...
| `Server.properties` | DONE | `spawn-protection 16` + `white-list` + `online-mode` + `enforce-secure-profile` + `view/simulationDistance` + `motd/seed/levelType/difficulty/resource-pack` + **plan35 `pvp/allow-flight/hardcore/max-players`** parsed in `main.cpp:37` + `GameServer.hpp:378`; `white-list`/`ops.json` enforced, `pvp`/`flight`/`hardcore` gamerule 同期済み。 |
...
Run: `cmake -B build && cmake --build build -j4 && ./build/test_smoke_80 ./build/cppfm` — currently `127 PASS 0 FAIL` (80/80 DONE + plan32-35 拡張 58). Polish `Trial Chambers/Pale Garden/Creaking` implemented in plan29; plan32-35 100点化 (WorldGen 7型/MultiNoise/20構造, Mob AI 10種, 6パケット, fuzz/soak, story20/predicate8) green at `9cba7f4` `spec 233`; remaining non-80 polish deferred: `Bundles` 1.21.5 (proto 776), `BossBar` lerp (verified client-side, §8), Mob AI 139→10 / 構造物ピース簡略 / perf async I/O (honest 90/100)。
