...
| **C-08** | Combat | Enchant 32→41 残 9 種の効果完全性 | `src/game/EnchantmentHelper.hpp:15` 13 accessors + generic `level()`, `src/game/CombatManager.cpp:35` EPF, `src/game/DamageSource.hpp:105` armor 式, `src/game/LootTables.hpp:75` `fortuneLevel`, `src/game/GameServer_items.cpp:483` horse/vehicle | Yarn `Enchantment` 41; wiki `Enchanting` 効果表 | 32/41Claim だが `static` accessor は 13 本 + `level()` 汎用で、効果適用が `CombatManager`/`LootTables`/`GameServer_items` に散在。残 9 種は `smite`/`bane_of_arthropods` の mob type 分岐、`punch` の knockback 距離、`flame` の着火 tick、`knockback` の velocity、`luck_of_the_sea`/`lure` の fishing、`multishot`/`piercing`/`quick_charge` の crossbow、`aqua_affinity`/`respiration` の水中。`mending` の XP 修繕・`infinity` の矢消費免除は plan37 で追加済みだが unit 網羅なし | **MEDIUM** | **OPEN** |
| **C-09** | Perf | Bench 厳密化 (p50/p95/hitRate assert) + LRU hit 公開 | `src/game/GameServer.hpp:922` `chunkCache LRU 1024`, `97` `ioWorkerThreads 4`, `tools/bench_chunk_gen.py:1` synthetic bench, `tests/test_smoke_80.cpp:936` `bench overworld view >=25` | Yarn `ThreadedAnvilChunkStorage` async I/O + `ChunkTicketManager`; wiki `View distance` 32 burst 16/tick; assessment-3 B-07 | LRU 1024 は plan38 で FIXED (`clear()` 撤廃) だが bench は `bench_chunk_gen.py` の `sleep(1-3ms)` synthetic で実 I/O を計測していない。`p50<5ms/p95<10ms/hit>80%` の assert が `test_smoke_80` / `SOAK_REPORT` に無い。`GameServer::stats` に `hitRate` / `queueSize` の公開 API がなく、`SOAK_REPORT` の `hitRate 84%` は clear 時代の推定値のまま。D +0.5 の半分 | **MEDIUM** | **OPEN** |
| **C-10** | Protocol | 未送信 21 の再分類と実装判断 | `src/proto/Ids.hpp:1` 131 `toClient` (plan34 で 6 パケット `0x18/0x20/0x25/0x50/0x51/0x6E` 追加済み), `docs/PROTOCOL_NOTES.md:1` 114 行, `src/game/GameServer_items.cpp:68` `handleMoveVehicle`, `95` `handleHorseJump`, `src/game/GameServer.hpp:759` `handleMoveVehicle/handleHorseJump`, `tools/score_review.py:27` `Unsent 27 documented` | Prismarine `protocol.json` 131 `toClient` vs 実送信 `~110`; Yarn `HorseEntity` / `MapState` / `Minecart` | plan34 で 6 パケット (ActionBar 0x51/ServerData 0x50/HurtAnimation 0x25/EntitySoundEffect 0x6E/ChatSuggestions 0x18/SyncEntityPosition 0x20) を FIXED。残 21 は `HorseWindow` (horse inventory 0x2A) / `MapData` 0x2C / `Minecart` 旋回 / `VehicleMove` 0x20 の server→client 側 / `OpenHorseWindow` 等。horse/map/minecart は vanilla 体験で可視欠落、演出系 (e.g. `Particle` 拡張) は文書化で足りるが再分類が未実施 | **MEDIUM** | **OPEN** |
| **C-11** | Inventory | レシピ 1578 の実クラフト網羅検証 | `src/game/Recipes.hpp:87` `matchShaped` + `107` `mirrored` triple loop, `src/game/Recipes.cpp:45` `syncTagsFrom`, `171` `TagManager` 同期, `assets/data/recipes/*.json` 1578, `src/game/TagManager.hpp:1` 67 tags | Yarn `ShapedRecipe#matches` 全 offset + mirror 探索; wiki `Crafting` mirror; `minecraft-data recipes` 1581 (1581 vs 1578 は 3 件の空白 `pattern` 除外) | `matchShaped` は `oy→ox→mirrored` triple loop で plan37 FIXED、`syncTagsFrom` も FIXED。だが 1578 の実地検証は `test_smoke_80` の `mirrored give (weak)` 1 件のみで `||true` のため網羅保証なし。全 shaped の `mirror`/`offset` を unit で網羅するテストが無い。3 件の `pattern ["   "]` 除外が 1581 vs 1578 の差だが除外理由の文書化なし | **LOW** | **OPEN** |
| **C-12** | Operational | 実クライアント互換の総合検証環境整備 | `tests/test_smoke_80.cpp:1` 188 checks, `tests/test_spec_wire.cpp:1` 268 checks, `tests/test_fuzz.cpp:1` 23, `tests/multi_client_test.py:1` 2-clients, `tests/soak_test.py:1` synthetic, `docs/SOAK_REPORT.md:1` 66 行 | Mojang vanilla client 1.21.4 + Fabric 1.21.4 の `ClientPlayNetworkHandler` tick 20; assessment-3 B-06/B-07 の soak/bench | `multi_client_test.py` は chat/block の 2-clients のみで `entity tracker` / `inventory drag mode5` / `view-distance 32` の網羅なし。CI は `ctest -R native|spec_wire|fuzz` のみで `smoke80` は manual (`timeout 450`)。rcon なし。vanilla client を用いたリグレッション CI (例: `mcproto` Bot の 20 TPS 1h) が未整備で、手動 Soak (C-04) との二重化なし。D +0.5 の残り | **LOW** | **OPEN** |
...
## 5. Datapack — Loot Tables 38→100 (C-05)
...
## 6. Datapack — Advancement Triggers 10→12 + ツリー 50→80 (C-06)
...
## 7. Datapack — Predicate 17→22 種 (C-07)
...
## 8. Combat — Enchant 32→41 残 9 種 (C-08)
