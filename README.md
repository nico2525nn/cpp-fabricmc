...
| Optional embedded JVM: HotSpot/JNI, opaque handles, shadow API, selected events, version-locked structural Mixin transformer, and selective routing | ⚠️ plan51 focused boundary only; the Mojang GameProvider and arbitrary mods are unsupported; pinned official Loader/Knot is verified by a separate offline probe |
...
- **JVM boundary:** `--jvm=true` can start optional embedded HotSpot and execute the repository's shadow ABI, structural transformer, and fixture corpus. The production path is not the Mojang GameProvider; arbitrary JVM mods remain unsupported. A separate offline probe verifies pinned official Loader/Knot/Mixin against the shadow provider; "Fabric-compatible" still means *protocol-compatible with what an unmodded Fabric server puts on the wire*.
