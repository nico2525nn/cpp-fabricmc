...
- **True parity — strict wire audit** (`docs/COMPAT_AUDIT_1_21_4_STRICT.md`, Web Search/Web Fetch verified, `file:line` absolute): **38 gaps remain of 78** (HIGH 10 fixed in `plan15`, MEDIUM 10 in `plan16`, LOW 10 in `plan17`, `plan18` 10 in progress). This doc is the true roadmap — see it for `Density zScale`, `pale_garden`, `Trial Chambers`, `Ids off-by-one`, `Bundle axis`, etc.
...
- **Strict audit (`docs/COMPAT_AUDIT_1_21_4_STRICT.md`):** `40/78` fixed, `38` remain. This is the evidence for true parity (Density pipeline, `pale_garden`, `Ids` wire, `Bundle` axis, etc.).
