...
## 3A. GitHub Actions / Pull Request 運用（必須）

- 以後の実装・修正・リファクタリング・ドキュメント更新は、必ず作業ブランチから GitHub Pull Request (PR) を作成して提出する。`main` への直接 push、直接 commit、ローカルだけでの完了扱いは禁止する。
- PR には変更理由、対象範囲、テスト手順、既知の制限、必要な証跡を記載し、レビュー可能な小さな単位に分ける。サブエージェントの変更も同じPRに統合してから提出する。
- PR作成後は `.github/workflows/` の GitHub Actions を必ず実行し、必須ジョブ（configure/build、対象テスト、lint/quality checks）が成功するまで修正と再実行を行う。Actions が未実行・失敗・キャンセルのPRはマージしない。
- マージ、rebase、release向けタグ付けは GitHub 上のPR保護ルールとActionsの成功結果を通して行う。ローカルの `git merge` は検証用worktree内に限り、`main` の統合手段にはしない。
- **条件付きマージ方針（全PR共通）**: このリポジトリのPRは、全差分をレビューしたうえで実装品質に確信があり、重大な未解決の正確性・セキュリティ・データ保全上の懸念がなく、当該head SHAの必須Actionsとブランチ保護条件がすべて成功している場合に限り、GitHub経由でマージしてよい。レビューコメントや競合などの未解決条件がある場合、または品質に確信が持てない場合はマージせず、懸念点を報告する。積み上げPRでは対象PRの指定baseにのみ統合し、親PRや `main` への統合は別途そのPRを評価してから行う。
- ローカルのビルド・テストはPR提出前の短時間な検証として実施してよいが、GitHub Actionsの結果を置き換えない。Actionsで再現できない結果はPR本文にコマンド、ログ、環境、制限を明記する。
- Actions設定やPR保護ルール自体を変更する場合も、別PRでレビューし、変更前後のActions実行結果を保存する。秘密情報・トークン・個人環境の設定をリポジトリへ書き込まない。
