...
def format_rows(rows, width=4):
    """Pack generated initializer rows while retaining stable formatting."""
    return "\n".join(
...
data = load_json(PRISMA_BLOCKS)
mcmeta = load_json(MCMETA_BLOCKS)
try:
    items_data = load_json(PRISMA_ITEMS)
...
legacy_entries = format_rows(['{"%s", %d},' % (n, s) for n, s in rows])
...
prop_def_rows = format_rows(
    ['{"%s", %d, %d},' % (n, ids[0], len(ids)) for n, ids in prop_defs])
value_rows = format_rows(
    ['"%s",' % v.replace('"', '\\"') for v in value_pool], width=8)
